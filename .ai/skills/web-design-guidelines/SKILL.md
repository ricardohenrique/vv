...
- Apply the repository `AGENTS.md`, existing React/Inertia components, and Tailwind CSS 4 tokens before generic examples in this skill. Do not add a UI library, animation library, router, or client-validation package merely to satisfy an example.
- Audit browser and Inertia interfaces as web documents. Audit NativePHP Mobile views against the installed NativePHP and Native UI component contracts; do not assume that every `<native:*>` component exposes ordinary HTML semantics.
- Laravel validation, authorization, and API error responses remain authoritative. Client-side and inline validation may improve feedback, but must not become the only enforcement layer or duplicate rules that can drift from the server contract.
- Prefer focused `motion-reduce:*` utilities or component-level fallbacks over a global universal selector that suppresses every transition. Preserve focus, progress, loading, and state-change feedback.
- Source review can identify likely accessibility defects, but it cannot establish complete WCAG conformance. Reserve a conformance conclusion for an audit that also includes rendered keyboard, screen-reader, contrast, zoom, reduced-motion, and representative mobile testing.
