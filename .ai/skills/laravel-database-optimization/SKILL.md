...
description: Laravel database optimization patterns. Use when writing Eloquent queries, creating migrations, configuring caching, debugging slow queries, or optimizing database performance. Triggers on tasks involving N+1 queries, indexes, pagination, transactions, query plans, or cache-backed reads. Apply driver-specific advice only after inspecting the configured database, cache store, installed packages, and production constraints.
license: MIT
metadata:
  author: agent-skills
  version: "1.1.1"
...
- Implementing cache-based optimizations supported by the configured store
...
- Debugging slow queries with the current database's query-plan tooling

## Repository guardrails

- Inspect `config/database.php`, `config/cache.php`, dependency manifests, the
  current schema, and the production database engine before selecting a rule.
- SQLite, MySQL, and PostgreSQL differ. Do not copy concurrent-index, full-text,
  locking, or `EXPLAIN ANALYZE` syntax across engines.
- Cache tags require a supported store such as Redis or Memcached. This
  boilerplate defaults to the database cache and therefore must not use tags.
- Do not install Laravel Debugbar, Redis, Telescope, or another observer/cache
  dependency unless it already exists or the user explicitly approves it.
- Measure a real query and inspect its plan before adding indexes, raw query
  builder hot paths, caching, or automatic eager-loading behavior.
- Prefer explicit `with()` plus non-production lazy-loading prevention. Do not
  enable global automatic eager loading as an unmeasured blanket fix.
...
| 4 | Caching (driver conditional) | HIGH | `cache-` |
...
### 4. Caching (HIGH, driver conditional)
...
### Cache Expensive Queries with the Configured Store
