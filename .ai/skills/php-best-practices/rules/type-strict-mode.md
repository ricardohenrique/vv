...
`declare(strict_types=1)` enforces strict type checking for calls made from that file. It is a project-level convention, not a safe one-file cleanup: inspect sibling files and repository instructions first. Add it to new files only when the project already uses strict types or when the user has approved a consistent migration. Do not mass-add it to Laravel skeleton files in this repository.
