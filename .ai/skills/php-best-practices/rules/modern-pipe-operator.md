...
Use the pipe operator (`|>`) for readable function chaining only when `composer.json` requires PHP 8.5 or newer. A PHP 8.5 local runtime is insufficient when the package constraint still permits PHP 8.4; use Laravel helpers, collections, or ordinary calls in that case.
