...
**Always check the project's supported PHP constraint before giving any advice.** Features vary significantly across 8.0 - 8.5. `composer.json` is authoritative: a newer local runtime does not permit syntax that the lowest supported version cannot parse.
...
Also check the runtime version for tooling compatibility, but do not treat it as the application's minimum:
```bash
...
**Only suggest features available in every version allowed by the Composer constraint.** For this repository, `^8.4` means PHP 8.4 syntax is the portable ceiling even when the current CLI is PHP 8.5. If the user asks about upgrading or newer features, mention what becomes available at each version.

Before applying a generic rule, inspect the Laravel skeleton and sibling files. Project conventions override this skill. Do not mass-add `declare(strict_types=1)` to a codebase that does not use it, and do not introduce property hooks into Eloquent models as a replacement for Laravel casts or accessors.
...
- `type-strict-mode` - Follow the project's strict-types convention consistently
...
**8.5+:**
- `modern-pipe-operator` - Pipe operator (`|>`) only when Composer requires PHP 8.5+
